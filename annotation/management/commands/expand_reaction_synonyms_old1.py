...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        synonym_list_orig = Reaction_synonym.objects.all()\
            .values_list('synonym','reaction__name','source__name')
        ...
        for entry in synonym_list_orig:
            ...
            no_bar = re.sub("\-\_","",synonym)
            
            if no_bar != synonym:
            
                dict_append(synonym_dict_new,no_bar,(rxn_id,source))
            
            no_brack = re.sub("\(\)","",synonym)
            
            if no_brack != synonym:
            
                dict_append(synonym_dict_new,no_brack,(rxn_id,source))
            
            no_brack_bar = re.sub("\(\)","",no_bar)
            
            if no_brack_bar != synonym:
            
                dict_append(synonym_dict_new,no_brack_bar,(rxn_id,source))
        ...
        for new_synonym in synonym_dict_new:
            ...
            if new_synonym not in synonym_syn_list:
                ...
                if len(rxn_dup_removed) == 1:
                    ...
                    try:
                        rxn = Reaction.objects.get(name=entries[0][0])
