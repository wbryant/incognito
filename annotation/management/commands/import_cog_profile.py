...
class Command(NoArgsCommand):
	...
	##	valid_gene = True
	##	print 'Gene %s found ...' % locus_tag
	##    except:
	##	print 'Gene %s not found ...' % locus_tag
	##    
	##    if valid_gene:
	##	gene.cog_id = cog_id
	##	gene.save()
	##	list_of_cogs.append(cog_id)
	##
	##f_in.close()
	##
	##list_of_cogs = list(set(list_of_cogs))
	##
	
	list_of_cogs = Gene.objects.filter(cog_id__isnull=False).distinct().values_list('cog_id', flat=True).order_by('cog_id')
	
	#print list_of_cogs[0:10]
	
	p_value_cutoff = 0.01

        print 'Calculating MTG stats for MTG_cog_result (%s COGs to be checked) ...' % len(list_of_cogs)
        #Initiate counter
        num_tot = len(list_of_cogs)
        #num_tot = 20
	num_done = 0
        next_progress = 1
	...
	for cog_id in list_of_cogs:
	    
	    #print 'Calculating reaction results for COG %s ...' % cog_id
	    output = cog_rxn(cog_id, GAP_STATUS)
	    
	    for result in output["inclusive"]:
		if float(result['phgf']) <= p_value_cutoff:
		    mtg_cog_result = MTG_cog_result(
			cog_id = cog_id,
			seed_id = result['SubjectID'],
			type = 'i',
			p_value = float(result['phgf']),
			rank = result['Rank']
		    )
		    mtg_cog_result.save()
		else:
	    ...
	    for result in output["exclusive"]:
		if float(result['phgf']) <= p_value_cutoff:
		    mtg_cog_result = MTG_cog_result(
			cog_id = cog_id,
			seed_id = result['SubjectID'],
...
def cog_rxn(COG, GAP_STATUS):
	COGName=cog_info.objects.using('cog').filter(COG=COG).values_list("COGName",flat=True)[0].encode("ascii","ignore")
	profile = cog_info.objects.using('cog').filter(COG=COG).values_list("COGPhyloProfile",flat=True)[0].encode('ascii','ignore')
	rxnAssigned=[]
	if unicode(COG) in list(cog_info.objects.using('cog').values_list("COG",flat=True).distinct()):
		rxnAssigned=cog_info.objects.using('cog').filter(COG=COG).values_list("Associatedrxn",flat=True)[0].encode('ascii','ignore')
...
def cog_rxn_profile(COG,COGName,profile, GAP_STATUS):
	seed_list = [SEED.encode("ascii","ignore") for (SEED,Species) in organisms.objects.using('cog').values_list("SEED","Species")]
	species = dict(organisms.objects.using('cog').values_list("SEED","Species").distinct())
	...
	eggnog_SEED = {}
	eggnog_Seq = {}
	for (GI,SEED,Seq) in eggnog.objects.using('cog').filter(COG=unicode(COG)).values_list("GI","SEED","Seq"):
		eggnog_GIs.append(GI)
		eggnog_SEED[GI] = SEED
		eggnog_Seq[GI] = Seq

	rxnlinks2 = {}
	for s,r,e in known_rxncog_association.objects.using('cog').filter(COG=COG).values_list("SEEDPeg","rxn","e"):
		exrxn,exe = rxnlinks2.setdefault(s,["",100])
		e = float(e)
		if e < exe and e < 1e-26:
			rxnlinks2[s] = [r,e] 

	rxnlinks1 = {x:(y,z) for x,y,z in peg_info.objects.using('cog').filter(GI__in=eggnog_GIs).values_list("GI","SEEDPeg","rxn")}
...
def cog_rxn_searches(Q,COGName,COGPhyloProfile, GAP_STATUS):
	...
	COGAll=cog_info.objects.using('cog').exclude(COGPhyloProfile=unicode("0"*len(organisms.objects.using('cog').all()))).values_list("COG",flat=True).distinct()
	#A list of COGs already assigned (by SEED model) to the investigated rxn
	COGAssigned=[]
	if unicode(Q) in list(rxn_info.objects.using('cog').values_list("rxn",flat=True).distinct()):
		COGAssigned=rxn_info.objects.using('cog').filter(rxn=Q).values_list("AssignedCOG",flat=True)[0].encode('ascii','ignore')
		COGAssigned=COGAssigned.split(",")

	#A list of all "informative i.e. non empty phyloprofile" rxns in the DB
	rxnAll=list(set(rxn_info.objects.using('cog').exclude(rxnPhyloProfileOriginal=unicode("0"*len(organisms.objects.using('cog').all()))).values_list("rxn",flat=True).distinct()))
	
	#A list of rxns already associated (by SEED model) with the investigated COG
	rxnAssociated=cog_info.objects.using('cog').filter(COG=Q).values_list("Associatedrxn",flat=True)[0].encode('ascii','ignore')
	...
	rxnPhyloProfileOriginals=dict(rxn_info.objects.using('cog').values_list("rxn","rxnPhyloProfileOriginal"))
	rxnNames=dict(rxn_info.objects.using('cog').values_list("rxn","rxnName"))
	Assocs = {x:(y,z) for x,y,z in rxn_info.objects.using('cog').values_list("rxn","AssignedCOG","AssignedCOGCount")}
	...
	for Masking in MaskingLst:
		...
		for i in range(0,len(rxnAll)):
			...
			#Profile comparison
			if MaskedrxnPhyloProfile != "0"*len(organisms.objects.using('cog').all()):
